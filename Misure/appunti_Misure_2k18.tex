...
È il più veloce convertitore A/D con $ T_{mis} = 1\cdot T_c $ (la misura occupa un colpo id clock) e raggiunge frequeze di conversione fino a 50 $ \frac{GSa}{s}$.
...
\begin{figure}[H]
  ...
  \end{center}
\end{figure}
Devo distinguiere tante soglie di comparazione che sono mandate agli ingressi degli opamp usati da comparatori di tensione con Vx che viene mandata in parallelo a tutti i comparatori. Divido quindi in n soglie equivalenti. Di stati possibili di commutazione tra gli uni e li zeri ci sono 255 casi possibili.
I comparatori sono messi in modo che fino ad un certo punto avranno in uscita 1 e dopo un certo punto avranno uscita 0. Allora il codificatore binario riesce a scrivere tutto in uscita digitale. Questo avviene tutto in un colpo di clock.
Parti da Vref e metti tutti i resistori uguali fino a 0. Per costruire un convertitore Flash \textbf{bipolare} conviene dimezzare il quanto di tensione intorno a 0 e mettere dei reistori 1/2 e 3/2 R e i conti tornano fiddatti.
È imortante sapere le formule seguenti perché vengon ospesso chieste nel pre-compito dell'esame. 
\textcolor{red}{QUINDI COPIA STE FORMULE A PAG 34/81}.
Usato in tutte le misure in cui la velocità è un parametro importante. Copiaci anche sta 35. Ocio che alla fine dei lucidi chè tutto un recap delle cose più importanti per ogni lucido FIGO.
\subsubsection{Esercizio Convertitore Flash}
Dinamica $ = \pm 10~V $ quindi la \textbf{dinamica} che dobbiamo usare nell'esercizio è $ 20~V $!

$ n = 8 $ bit (perché questi ha il Flash).

$ f_{sample} = 1~\frac{GSa}{s} $

\textcolor{red}{E copiaci anche sto esercizio.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
