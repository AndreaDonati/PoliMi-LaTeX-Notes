...
Caratteristici per essere lenti, ma molto accurati. Completamente diversi dai differenziali perchè non sfruttano una differenza, ma il loro circuito interno calcola il risultato secondo l'integrale del segnale d'ingresso e dei rumori.
\begin{equation}
...
$$
La particolraità di questa relazione è:
\begin{equation}
  V_m \propto \frac{1}{T_I}\int_0^{T_I}[V_x + V_d(t)]dt = V_x + \frac{1}{T_I}\int_0^{T_I}V_d(t)dt
\end{equation}
Per quanto riguarda l'ultimo termine ($\frac{1}{T_I}\int_0^{T_I}V_d(t)dt$) noi abbiamo l'obiettivo di renderlo trascurabile.

INTEGRARE IL DISTURBO SU UN MULTIPLO INTERO DEL SUO PERIODO MI DA SICURAMENTE 0!!! Quindi se $ T_I $ lo metto esattamente come un multiplo del periodo del disturbo allora l'ho fottuto tutto.
Se invece non riesco a fare esattamente il multiplo perchè non conosco esattamente la frequenza (il periodo) del disturbo allora mi rimane una parte, ma la parte che rimane È PICCOLA.
