...
Caratteristici per essere lenti, ma molto accurati. Completamente diversi dai differenziali perchè non sfruttano una differenza tra il valore in ingresso ed un riferimanto interno, ma il loro circuito calcola il risultato secondo l'integrale del segnale d'ingresso e dei rumori.
\begin{equation}
...
\end{equation}
Per quanto riguarda l'ultimo termine ($\frac{1}{T_I}\int_0^{T_I}V_d(t)dt$) noi abbiamo l'obiettivo di renderlo trascurabile, attraverso alcune strategie ed impostazioni di tempo (tempo di integrazione $ T_I $).
...
Fissata la frequenza del disturbo sappiamo scegliere il tempo di integrazione perfetto per eliminare tutti i disturbi a tale frequenza, mentre fissato il tempo di integrazione sappiamo le frequenze dei disturbi che possiamo elminare.
