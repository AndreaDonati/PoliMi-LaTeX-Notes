...
\begin{abstract}
Appunti presi a lezione durante l'AA 2018/2019. Questa raccolta \textbf{non} è da considerarsi un sostitutivo del materiale fornito dal professore, alla frequentazione delle lezioni od ai libri di testo, ma è da prendere piuttosto come materiale integrativo.\\
La raccolta può risultare incompleta/inesatta in alcuni passaggi, quindi ogni suggerimento o correzione è apprezzato.
\end{abstract}
\newpage
...
\section{Variabilità delle Misure} Misure ripetute dello stesso parametro fisico non forniscono lo stesso valore. È considerabile come una legge della natura, è impossibile che due misure consecutive diano esattamente lo stesso risultato per ogni cifra decimale. La caratteristica che ci permette di determinare l'incertezza della misura è la \textbf{deviazione standard} (o dispersione $\sigma$) della misura in questione, mentre il valore della misura sarà la \textbf{media} ($\mu$).\footnotemark[1]\footnotetext[1]{Per la spiegazione teorica di \textit{media} e \textit{deviazione standard} si rimanda alla sezione \ref{probStat}.}

Le misure sono sempre affette da fluttuazioni o errori (almeno potenziali), mai perfettamente conoscibili, che si traducono in una naturale "indeterminazione" o \textbf{incertezza} sul risultato di misura.\\
...
\end{itemize}
\section{Richiami di Probabilità e Statistica} \label{probStat}
...
\begin{center}
  \textit{Come scegliamo l'intervallo e la PDF?}
\end{center}
...
$$
che risulta essere la somma pesata, con pesi $c_i^2$, delle incertezze (varianze) degli ingressi $x_i$ più la somma dei termini di \textit{covarianza}, sempre pesati con le derivate prime della relazione funzionale.
...
$$
  r_{ij}(x_i,x_j) \equiv \frac{u(x_i,x_j)}{u(x_i)\cdot u(x_j)}~\epsilon [-1;+1]~
...
\begin{equation}
  \label{media pesata misure compatibili}  
    x = \bar{x}_{MP} = \frac{\sum^N_{i=1}w_i x_i}{\sum^N_{i=1}w_i} = \frac{\sum^N_{i=1}\frac{x_i}{u^2(x_i)}}{\sum^N_{i=1}\frac{1}{u^2(x_i)}}
\end{equation}
...
\begin{equation}
  \label{incertezza media pesata misure compatibili}
    u^2(\bar{x}_{MP}) = \frac{1}{\sum^n_{i=1}w_i} = \frac{1}{\sum^n_{i=1}\frac{1}{u^2(x_i)}}
\end{equation}
...
%Gli ADC consentono di fare misure di grandezze fisiche fornendo il valore misurato in formato numerico, nei casi specifici che analizziamo andremo a studiare diversi ADC che misurano dei valori di tensione.
...
\end{align*}
E l'\textit{Amplificatore Operazionale} serve da sommatore delle correnti pesate che passano attraverso gli \textit{switch} e converte la corrente risultante $i_o$, attraverso la restistenza di \textit{feedback}, in un'uscita di tensione $v_o$.
...
In formule\footnotemark[1]\footnotetext[1]{È importante ricordare queste formule e il loro significato, anche perché è spesso necessario usarle per rispondere al pre-compito in sede d'eseme.}:
\begin{itemize}
...
Caratteristici per essere lenti, ma molto accurati. Completamente diversi dai differenziali perché non sfruttano una differenza tra il valore in ingresso ed un riferimento interno, ma il loro circuito calcola il risultato secondo l'integrale del segnale d'ingresso e dei rumori.
\begin{equation}
...
In formule:
\begin{align*}
  &\frac{1}{T_I}\int_0^{T_I}V_d(t)dt \rightarrow 0\text{~~per~~} T_I \rightarrow kT_{d}\text{~~dove~~} k \in \mathbb{N} 
\end{align*}
Se invece non riesco a settare $ T_I $ ad esattamente un multiplo perché non conosco esattamente la frequenza (e quindi il periodo) del disturbo allora mi rimane una parte e, in casi fortunati o studiati intelligentemente, questa parte è sufficientemente piccola da risultare trascurabile.

In generale comunque per $ T_I >> T_{d} $ la reiezione al disturbo migliora, infatti è vero che:
\begin{align*}
    &\frac{1}{T_I}\int_0^{T_I}V_d(t)dt \rightarrow 0\text{~~per~~} T_I \rightarrow \infty\text{~~oppure~~} T_I >> T_{d} 
\end{align*}
...
\begin{equation}
  V_x = - \frac{T_{d}}{T_{u}}V_r = - \frac{N_d}{N_u}V_r
\end{equation}
...
Quello che abbiamo chiamato $ T_{u} $ (T UP) è in realtà il \textbf{tempo di integrazione} ($ T_I $) e quindi $ T_{u} = N_u T_C $ è \textbf{costante} e \textbf{fissato}, mentre il $ T_{d} $ (T DOWN) è il tempo che convertiamo in tensione, che è il tempo che impiega il condensatore a scaricarsi a 0.

Quindi il tempo di misura totale è 
$$
  T_{mis} = T_{u} + T_{d}
$$
che è variabile con $ T_{d} $ (che varia proporzionalmente a $ V_x $) e dipende da $ V_x $ come nel voltmetro a rampa analogica.
...
Misura teoricamente insensibile a valori e incertezze di R, C ($ \tau = 1/RC $) e altri parametri strumentali ($ f_c $ e $ T_c $). Naturalmente l'\textbf{incertezza} di  $ V_r $ si trasferisce $ 1:1 $ su $ V_x $. L'unico errore presente nel voltmetro a doppia rampa è quello nel conteggio di $ N_d $ (errore di quantizzazione).
...
Richieste:
\begin{enumerate}
  \item Determinare la \textbf{velocità di lettura} in termini di $ f_{mis}\ \Big[ \frac{Sa}{s} \Big] $
...
\begin{enumerate}
  \item La velocità di lettura si esprime in termini di quanti campioni al secondo si possono analizzare, cioè quane misurazioni è possibile eseguire in un secondo. In un voltmetro a doppia rampa, il tempo di una singola misura $ T_{mis} $ è definito come:
  $$
    T_{mis} = T_{u}+T_{d}
  $$
  Dove $ T_{u} $ è il tempo di integrazione (che ricordiamo essere un parametro \textbf{fisso} stabilito dal costruttore dello strumento), conteggiato da $ N_u $ e $ T_{d} $ è il tempo impiegato dalla rampa di discesa per tornare a 0 ed è conteggiato da $ N_d $.
  
  Per calcolare la velocità di lettura dobbiamo effettuare una specie di approssimazione per eccesso: è necessario considerare il \textbf{massimo} $ T_{mis} $, in modo da non sovrastimare il risultato indicando una velocità di lettura sbagliata, mai raggiunta se si misurano segnali al limite della dinamica del voltmetro.
  
  Dato che $ T_{u} $ è fissato, il parametro che dobbiamo "massimizzare" è $ T_{d} $. Per stabilire il massimo tempo di discesa è necessario stabilire il massimo numero di conteggi che il contatore interno è in grado di eseguire durante la rampa di discesa: $ N{d,max} $. Questo parametro è \textbf{sempre} calcolabile come:
  \begin{align*}
    &N_{d,max} = 2^n\ \text{\ \ nel caso di dinamica \textbf{unipolare}}\\
    &N_{d,max} = 2^{n-1}\ \text{nel caso di dinamica \textbf{bipolare} per il bit di segno}\\
  \end{align*}
  In questo caso non sono state date indicazioni riguardo alla dinamica quindi la consideriamo \textbf{unipolare}. Calcoliamo quindi $ N_{d,max} = 2^n = 2^{16} = 65536 $ livelli (che equivalgono a 4 cifre e mezzo).
  
  Possiamo quindi calcolare $ T_{d,max} $, ricavando prima il periodo di \textit{clock} come $ T_c = \frac{1}{f_c} = 1\ \mu s$
  $$
    T_{d,max} = \frac{N_{d,max}}{f_c} = N_{d,max}\cdot T_c = 65536\ \mu s \cong 65\ ms
  $$
  
  Quindi $ T_{mis,max} = T_{u} + T_{d,max} = 165\ ms$ e da qui ricaviamo $ f_{mis} = \frac{1}{T_{mis_max}} \cong 6\ \frac{Sa}{s} $.
  
  \item Il numero di cifre decimali del contatore è definto come $ M_{contatore} = \log_{10}(\max{\{N_u,N_d\}}) $ perché il contatore deve essere in grado di soddisfare il numero di livelli di conteggio sia in salita sia in discesa. 
  
  Dobbiamo innanzitutto calcolare il valore di $ N_u $:
  $$ N_u = T_{u}\cdot f_{clock} = 100000 = 10^5 $$
  
  Nel nostro caso $ N_u > N_d $ e quindi:
  $$
    M_{contatore} = \log_{10}(N_u) = \log_{10}(10^5) = 5\ \text{cifre}
  $$
\end{enumerate}

\section{Bit Equivalenti}

Il termine \textit{bit equivalenti} nasce dalla necessità di distinguere il numero di bit \textit{reali} di un ADC ed il numero di bit \textit{effettivi} con cui si compie la misura. Questa distinzione è fondamentale nei casi in cui analizziamo segnali affetti da \textbf{rumori}.

Questi rumori possono essere \textit{interni} od \textit{esterni} all'ADC. Nel caso di rumori \textbf{interni} parliamo di quei rumori che sono generati dai riferimenti/componenti interni al convertitore, mentre i rumori \textbf{esterni} sono rumori dovuti al circuito che si sta analizzando.

Disponendo di un convertitore/voltmetro che quantizza il segnale $ s(t) $ su $ n $ bit, si avrà un \textit{passo di quantizzazione} $ Q = \frac{D}{2^n} $ (uguale alla risoluzione dello strumento). Se D è la dinamica del segnale e anche del voltmetro, si ha una varianza (“i\textit{ncertezza di quantizzazione}") pari a 

$$
  \sigma^2_q = \frac{Q^2}{12} = \frac{\sigma^2_s}{2^{2n}}   
$$
con $ \sigma^2_s $ varianza del segnale.\\
Da questa formula possiamo quindi dedurre che il rapporto tra la varianza del segnale e la varianza di quantizzazione è \textbf{costante}:

$$
  \frac{\sigma^2_s}{\sigma^2_q} = 2^{2n} \text{\ \ \textbf{costante}.}
$$
Per uno strumento con quantizzatore \textit{ideale}, allora:
$$
  \frac{S}{N_q} = 2^{2n}
$$
Per cui è possibile ricavare il numero $ n $ di bit per un convertitore ideale dal rapporto $ S/N_q $ (Signal/Noise) dove il rumore è dovuto solamente alla quantizzazione, tramite la formula:
\begin{align*}
  &n = \frac{1}{2}\log_2\Bigg(\frac{S}{N}\Bigg)\\
  &\text{con } S = \sigma^2_s\\
  &\text{e \ \ } N = \sigma^2_q\\
\end{align*}

Nel caso invece di un convertitore \textbf{reale} c'è da considerare, oltre al rumore di quantizzazione, anche il \textit{rumore aggiunto}, che è somma del rumore \textbf{interno} dell'ADC e del rumore \textbf{esterno}. Abbiamo quindi un rumore complessivo che può essere espresso come:
$$
  N_c = N_q + N_{ADC} + N_{ext} (> \text{del solo } N_q)
$$
In termini di varianza:
$$
  \sigma^2_c = \sigma^2_q + \sigma^2_{n,ADC} + \sigma^2_{n,ext} (> \text{del solo } \sigma^2_q )
$$
Definiamo quindi, nel caso \textbf{reale}, il \textbf{numero di bit equivalenti come}:
\begin{align*}
    &n_e \equiv \frac{1}{2}\log_2\Bigg(\frac{\sigma^2_s}{\sigma^2_c}\Bigg)\\
    \\
    &\Rightarrow n_e = \frac{1}{2}\log_2\Bigg(\frac{S}{N_c}\Bigg)
\end{align*}
Ed attraverso alcuni brevi passaggi possiamo arrivare a scrivere:
$$
  n_e = n-\frac{1}{2}\log_2\Bigg(1+\frac{\sigma^2_{n,ADC}+\sigma^2_{n,ext}}{\sigma^2_q}\Bigg)
$$
